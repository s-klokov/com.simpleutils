...
public class TimeConditionTrigger {
    ...
    public static TimeConditionTrigger newPeriodicTrigger(final long timeout, final ChronoUnit unit) {
        final ZonedDateTimeHolder deadline = new ZonedDateTimeHolder(ZonedDateTime.now().plus(timeout, unit));
        return new TimeConditionTrigger(now -> {
            if (!now.isBefore(deadline.zonedDateTime)) {
                deadline.zonedDateTime = now.plus(timeout, unit);
                return true;
            } else {
                return false;
            }
    ...
    /**
     * Класс-хранитель значения типа ZonedDateTime.
     */
    private static class ZonedDateTimeHolder {
        ZonedDateTime zonedDateTime;

        ZonedDateTimeHolder(final ZonedDateTime zonedDateTime) {
